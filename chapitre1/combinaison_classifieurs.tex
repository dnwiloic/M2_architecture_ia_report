...
\subsection{Définition et phases de la combinaison de classifieurs}
...
\subsubsection{Nature des sorties et niveaux d'information}
...
\subsection{Motivations théoriques et justifications statistiques}
...
\subsubsection{Théorème du jury de Condorcet}
...
\subsubsection{Décomposition biais--variance}
...
\subsubsection{Théorie unifiée de la diversité dans les ensembles}
...
\subsubsection{Les problèmes de Dietterich}
...
\subsection{Achirtectures des systèmes multi-classifieurs}
...
\subsubsection{Architectures de contrôle de flux}
...
\subsubsection{Systèmes de sélection dynamique (DS)}
...
\subsection{Typologies des méthodes d'ensemble}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{chapitre1/figures/Apercu_Random_Forest.png}
...
\paragraph{Limites du bagging.}
\begin{itemize}
  \item \textbf{Réduction limitée du biais} : le bagging agit principalement sur la variance ; si les
  apprenants de base sont fortement biaisés (arbres très peu profonds, modèles linéaires pour
  des relations hautement non linéaires, etc.), l'ensemble restera sous-ajusté.
  \item \textbf{Faible gain en cas de forte corrélation} : lorsque les modèles de base sont très corrélés
  (données peu diversifiées, caractéristiques dominantes, absence de randomisation suffisante),
  la réduction de variance reste limitée alors que le coût computationnel (entraînement et
  inférence multipliés par $m$) devient important.
  \item \textbf{Petits jeux de données et OOB instable} : sur de très petits jeux de données, le
  bootstrap peut produire des échantillons très similaires, rendant l'estimation OOB instable
  et réduisant l'intérêt pratique de la méthode.
\end{itemize}
...
\paragraph{Limites du boosting.}
\begin{itemize}
  \item \textbf{Sensibilité au bruit et aux outliers} : en se focalisant sur les exemples les plus
  difficiles, le boosting (en particulier AdaBoost et certains gradient boosting peu régularisés)
  peut sur-ajuster les observations bruitées ou mal étiquetées et dégrader la généralisation.
  \item \textbf{Risque de sur-apprentissage} : si l'on choisit des apprenants de base trop complexes
  ou un nombre d'itérations trop grand sans contrôle (pas d'\emph{early stopping}, régularisation
  faible, taux d'apprentissage trop élevé), le biais devient très faible mais la variance augmente
  fortement.
  \item \textbf{Coût de calcul et complexité de réglage} : les implémentations modernes comme
  XGBoost et LightGBM atténuent ces effets via la pénalisation des arbres, le sous-échantillonnage
  et un réglage fin des hyperparamètres, au prix d'une complexité de configuration et d'un coût
  de calcul parfois élevés.
  \item \textbf{Interprétabilité limitée} : comme pour la plupart des ensembles d'arbres, l'interprétation
  globale du modèle reste difficile malgré l'existence d'indicateurs d'importance de variables et
  d'outils d'explicabilité locale.
\end{itemize}
...
\paragraph{Démonstration pratique : \texttt{StackingClassifier} sur Iris.}
Le code ci-dessous illustre une démonstration reproductible du stacking en classification sur le
jeu de données Iris (trois classes, deux variables explicatives pour la visualisation). Trois
apprenants de base hétérogènes sont utilisés : $k$-NN ($k=1$), forêt aléatoire et classifieur
bayésien naïf gaussien ; le méta-modèle est une régression logistique (\texttt{mlxtend.classifier.StackingClassifier}). Le premier bloc charge les données, le second construit le \emph{StackingClassifier} et lance une validation croisée en 3 plis pour comparer l'exactitude de chaque modèle seul à celle de l'ensemble empilé : dans cette expérience, le stacking atteint la même précision que le meilleur base learner (forêt aléatoire, environ 95\,\%) tout en reposant sur une combinaison apprise des trois modèles. Un dernier bloc (optionnel) montre comment visualiser les régions de décision de chaque classifieur et de l'ensemble.

\begin{lstlisting}
  from sklearn import datasets
  
  iris = datasets.load_iris()
  X, y = iris.data[:, 1:3], iris.target\end{lstlisting}
  ...
  \textbf{Sortie:}\begin{verbatim}
    
  \end{verbatim}

  \begin{figure}[hbtp]
    \centering
    \includegraphics[width=0.8\textwidth]{chapitre1/figures/stackingExp4.png}
  \end{figure}

\subsubsection{Voting (vote majoritaire vs vote probabiliste)}
...
\subsection{Avantages et limites de la combinaison}
